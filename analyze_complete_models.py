...
# -*- coding: utf-8 -*-
"""
分析liblib.art汽车交通板块所有模型 (完整版 - 28个模型)
"""
...
def main():
    ...
    print("=== Liblib.art 汽车交通板块模型完整分析 (28个模型) ===\n")
    ...
    print(f"   独家模型: {int(df['isExclusive'].sum())}")
    print(f"   普通模型: {int((~df['isExclusive']).sum())}")
    print(f"   总浏览量: {int(df['views_num'].sum()):,}")
    print(f"   总点赞数: {int(df['likes_num'].sum()):,}")
    print(f"   总下载量: {int(df['downloads_num'].sum()):,}")
    print(f"   平均浏览量: {df['views_num'].mean():.1f}")
    print(f"   平均点赞数: {df['likes_num'].mean():.1f}")
    print(f"   平均下载量: {df['downloads_num'].mean():.1f}")
    ...
    for author, count in author_counts.head(5).items():
        print(f"     {author}: {count} 个模型")
    
    # 4. 浏览量排行榜
    print(f"\n4. 浏览量排行榜 (Top 10)")
    top_views = df.nlargest(10, 'views_num')[['title', 'author', 'views_num']]
    for i, (_, row) in enumerate(top_views.iterrows(), 1):
        print(f"   {i:2d}. {row['title'][:30]:<30} | {row['author']:<15} | {row['views_num']:>6}")
    
    # 5. 下载量排行榜
    print(f"\n5. 下载量排行榜 (Top 10)")
    top_downloads = df.nlargest(10, 'downloads_num')[['title', 'author', 'downloads_num']]
    for i, (_, row) in enumerate(top_downloads.iterrows(), 1):
        print(f"   {i:2d}. {row['title'][:30]:<30} | {row['author']:<15} | {row['downloads_num']:>6}")
    
    # 6. 点赞数排行榜
    print(f"\n6. 点赞数排行榜 (Top 10)")
    top_likes = df.nlargest(10, 'likes_num')[['title', 'author', 'likes_num']]
    for i, (_, row) in enumerate(top_likes.iterrows(), 1):
        print(f"   {i:2d}. {row['title'][:30]:<30} | {row['author']:<15} | {row['likes_num']:>6}")
    
    # 7. 数据相关性分析
    print(f"\n7. 数据相关性分析")
    correlation = df[['views_num', 'likes_num', 'downloads_num']].corr()
    print(f"   浏览量与点赞数相关性: {correlation.loc['views_num', 'likes_num']:.3f}")
    print(f"   浏览量与下载量相关性: {correlation.loc['views_num', 'downloads_num']:.3f}")
    print(f"   点赞数与下载量相关性: {correlation.loc['likes_num', 'downloads_num']:.3f}")
    ...
    for i, (_, row) in enumerate(top_engagement.iterrows(), 1):
        print(f"     {i}. {row['title'][:30]:<30} | {row['author']:<15} | {row['engagement_rate']:.2f}%")
    
    # 9. 独家模型分析
    print(f"\n9. 独家模型分析")
    exclusive_df = df[df['isExclusive'] == True]
    non_exclusive_df = df[df['isExclusive'] == False]
    
    print(f"   独家模型平均浏览量: {exclusive_df['views_num'].mean():.1f}")
    print(f"   普通模型平均浏览量: {non_exclusive_df['views_num'].mean():.1f}")
    print(f"   独家模型平均下载量: {exclusive_df['downloads_num'].mean():.1f}")
    print(f"   普通模型平均下载量: {non_exclusive_df['downloads_num'].mean():.1f}")
    ...
    output_data = {
        ...
        'models': models,
        'rankings': {
            'top_views': df.nlargest(10, 'views_num')[['title', 'author', 'views_num']].to_dict('records'),
            'top_downloads': df.nlargest(10, 'downloads_num')[['title', 'author', 'downloads_num']].to_dict('records'),
            'top_likes': df.nlargest(10, 'likes_num')[['title', 'author', 'likes_num']].to_dict('records'),
            'top_engagement': df.nlargest(5, 'engagement_rate')[['title', 'author', 'engagement_rate']].to_dict('records')
        },
        'statistics': {
    ...
    with open('complete_car_models_analysis.json', 'w', encoding='utf-8') as f:
        json.dump(output_data, f, ensure_ascii=False, indent=2)
    
    print(f"\n10. 数据导出")
    print("   详细分析数据已保存到: complete_car_models_analysis.json")
    
    print(f"\n=== 分析完成 ===")
    print(f"本次分析基于页面滚动发现的 {len(models)} 个汽车交通模型")
    print(f"相比之前的18个模型，新增了 {len(models) - 18} 个模型")
    print(f"数据完整性: {len(models)}/28 (100%)")
