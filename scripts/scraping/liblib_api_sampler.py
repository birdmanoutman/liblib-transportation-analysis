...
Usage examples:
  python scripts/scraping/liblib_api_sampler.py --pages 3 --max-details 10 \
    --out-dir data/raw/liblib/samples

Optionally provide a captured request payload (JSON file) for img/group/search:
  python scripts/scraping/liblib_api_sampler.py --payload-file payloads/list_payload.json
"""
...
def default_list_payload(page: int = 1, page_size: int = 24) -> Dict[str, Any]:
    """
    Reasonable default payload for img/group/search. Real payloads may differ.
    ...
    return {
        ...
        "page": page,
        "pageSize": page_size,
        "sortType": "newest",  # newest|hot|recommend
        "nsfw": False,
...
def fetch_list_pages(
    ...
    for page in range(1, pages + 1):
        ...
        url = f"{LIBLIB_API_BASE}/api/www/img/group/search?timestamp={int(time.time()*1000)}"
